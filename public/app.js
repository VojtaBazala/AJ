...
function checkPron(heard) {
  ...
  if (ratio >= 0.7) {
    ...
    speak('Well done! Next sentence.');
    setTimeout(startPron, 2000);
  } else if (ratio >= 0.4) {
    showFb('pronFb', 'info', `~ OK. Heard: "${heard}" — try again`);
    speak('Not quite. Listen again.', 'en-US', () => {
      speak(currentSent, 'en-US', () => setTimeout(listenPron, 600));
    });
  } else {
...
// ── VOCAB ──────────────────────────────────────────────────
const WORDS = [
  {cz:'electricity',en:'electricity'},{cz:'renewable',en:'renewable'},
  {cz:'forecast',en:'forecast'},{cz:'investment',en:'investment'},
  {cz:'consumption',en:'consumption'},{cz:'transition',en:'transition'},
  {cz:'supply and demand',en:'supply and demand'},{cz:'energy crisis',en:'energy crisis'},
  {cz:'tariff',en:'tariff'},{cz:'emissions',en:'emissions'},
  {cz:'grid',en:'grid'},{cz:'battery',en:'battery'},
  {cz:'capacity',en:'capacity'},{cz:'market',en:'market'},
  {cz:'subsidy',en:'subsidy'},{cz:'storage',en:'storage'},
  {cz:'pipeline',en:'pipeline'},{cz:'output',en:'output'},
  {cz:'contract',en:'contract'},{cz:'regulation',en:'regulation'},
  {cz:'volatility',en:'volatility'},{cz:'infrastructure',en:'infrastructure'},
  {cz:'sustainable',en:'sustainable'},{cz:'efficiency',en:'efficiency'},
  {cz:'peak demand',en:'peak demand'},{cz:'carbon neutral',en:'carbon neutral'},
  {cz:'negotiate',en:'negotiate'},{cz:'deadline',en:'deadline'},
  {cz:'acknowledge',en:'acknowledge'},{cz:'forecast',en:'forecast'}
];
...
function showVocab() {
  ...
  speak(w.cz, 'en-US', () => setTimeout(listenVocab, 500));
}
